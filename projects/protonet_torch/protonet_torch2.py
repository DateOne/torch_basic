...
parser.add_argument(
	...
	default=0.001)
parser.add_argument(
	'-ctr', '--num_classes_iter_tr', type=int,
	help='number of classes per iteration during training',
	default=60)
parser.add_argument(
	'-str', '--num_support_samples_tr', type=int,
	help='number of support samples per class per iteration during training',
	default=5)
parser.add_argument(
	'-qtr', '--num_query_samples_tr', type=int,
	help='number of query samplers per class per iteration during training',
	default=5)
parser.add_argument(
	'-cval', '--num_classes_iter_val', type=int,
	help='number of classes per iteration during validation',
	default=5)
parser.add_argument(
	'-sval', '--num_support_samples_val', type=int,
	help='number of support samples per class per iteration during validation',
	default=5)
parser.add_argument(
	'-qval', '--num_query_samples_val', type=int,
	help='number of query samples per class per iteration during validation',
	default=15)
...
class FSLBatchSampler(object):
	'''
	customized batch sampler for few-shot learing
	(in this edition, specially for Omniglot dataset)
	__init__, __iter__, __len__ methods inheritaed from object
	caution: for batch sampler in torch, __iter__ yield!
	'''
	def __init__(self, data_source, num_classes, num_samples, iterations):
		...
		self.dset_num_sample_per_class = 20
		self.dset_num_samples = len(self.data_source)
		self.dset_num_classes = self.dset_num_samples / self.dset_num_sample_per_class
	def __iter__(self):
		batch = []
		class_idcs = torch.randperm(self.dset_num_classes)[0 : self.num_classes]
		for class_idx in class_idcs:
			sample_idcs = torch.randperm(self.dset_num_sample_per_class)[0 : self.num_samples]
			for sample_idx in sample_idcs:
				batch.append(self.dset_num_sample_per_class * class_idx + sample_idx)
		yield batch
	def __len__(self):
